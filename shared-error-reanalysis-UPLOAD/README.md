...
```r
# Install the required CRAN packages (run_all.R lists and checks them)
install.packages(c("dplyr", "tidyr", "purrr", "stringr", "stringi",
                   "forcats", "broom", "ggplot2", "patchwork", "here"))
...
R/
  ...
  05_figures.R            R diagnostic versions of Figs 1-2 (F1_*.png, F2_*.png);
                          the manuscript figures come from scripts/make_figures.py
  06_inference.R          Exceedance tests, cluster-robust slopes, Wilson
                          intervals, the attribution bound (SI 2.8)
scripts/
  make_figures.py         The manuscript figures (fig1-3.png); the same code is
                          embedded, eval-guarded, in docs/analysis_report.Rmd
tests/
...
output/
  figures/                fig1-3.png (manuscript), F1/F2 (diagnostic)
  tables/                 Supplementary Tables 1 and 2, reported_values.csv,
                          exceedance_tests.csv, consistent_reporting.csv
docs/
  analysis_report.Rmd/.html      Self-verifying analysis report: recomputes every
                                 quoted value and refuses to render on a mismatch
  supplementary_information.Rmd  Generates the journal SI (Word); all numbers
                                 pulled from output/tables/
  reproducible_report.Rmd        Narrated walk-through of the whole analysis
```
...
- Condition names are recoded to ASCII before regression and string comparisons
  use a fixed ICU collator, `run_all.R` switches non-UTF-8 sessions to a UTF-8
  locale, and every seeded loop iterates in file order rather than sorted
  order, so results are identical under any locale and on any machine.
- `output/sessionInfo.txt` is written on every run, and the exact package
  versions for a verified end-to-end run are recorded in the Session
  information section of `docs/analysis_report.html`.
