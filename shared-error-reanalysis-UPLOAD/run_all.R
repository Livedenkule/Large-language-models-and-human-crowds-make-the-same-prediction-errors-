...
# Some condition names contain typographic quotes; in a non-UTF-8 locale
# (e.g. the C locale of many Linux containers) R rewrites them as literal
# "<U+201C>" text when writing the deposited CSVs, so the files depend on
# the machine that produced them. Ensure a UTF-8 locale for this session.
if (!l10n_info()[["UTF-8"]]) {
  for (loc in c("C.UTF-8", "C.utf8", "en_US.UTF-8"))
    if (nzchar(suppressWarnings(Sys.setlocale("LC_CTYPE", loc)))) break
  if (!l10n_info()[["UTF-8"]])
    warning("Non-UTF-8 locale; deposited CSVs may not be byte-identical.")
}
...
analysis_scripts <- c("03_main_results.R", "04_robustness.R",
                      "06_inference.R", "05_figures.R")
