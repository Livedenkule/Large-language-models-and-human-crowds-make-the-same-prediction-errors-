...
#'
#' Used for conditional shares whose denominator is a subset of comparisons
#' (e.g. "of GPT-4's misses, the share the crowd also missed"). For the
#' headline archive-wide rates the study bootstrap from 03_main_results.R is
#' the preferred interval because comparisons cluster within studies; the
#' Wilson interval is reported alongside it for the reader.
#'
#' @param k Successes. @param n Trials. @param level Coverage.
#' @return c(lo, hi) as proportions.
...
#' Cluster-robust variance matrix for an lm fit
#'
#' Companion to cluster_se() in 00_functions.R, returning the full matrix so
#' that a confidence band for the fitted line can be drawn in Fig. 1a.
...
#' Fitted line with a cluster-robust 95% band, on a grid of x values
slope_band <- function(x, y, cluster, grid = NULL) {
  fit <- stats::lm(y ~ x)
  V   <- cluster_vcov(fit, cluster)
  if (is.null(grid)) grid <- seq(min(x), max(x), length.out = 200)
  G   <- cbind(1, grid)
  se  <- sqrt(rowSums((G %*% V) * G))
  yh  <- as.vector(G %*% stats::coef(fit))
  list(slope    = stats::coef(fit)[2],
       slope_se = sqrt(V[2, 2]),
       band     = data.frame(x = grid, fit = yh,
                             lo = yh - 1.96 * se, hi = yh + 1.96 * se))
}
...
if (!exists(".reported", inherits = TRUE)) {
...
#' write.csv that always produces a UTF-8 file, independent of the OS locale
#'
#' In a non-UTF-8 locale (e.g. the C locale of many Linux containers), plain
#' write.csv() re-writes characters it cannot represent -- such as the
#' typographic quotes in some condition names -- as literal "<U+201C>" text,
#' so the deposited file depends on the machine that produced it. Writing
#' through an explicit UTF-8 connection makes the bytes identical everywhere.
