...
# This reanalysis uses all 1,678 available comparisons. The authors instead
# draw one reference condition per study, compare every other condition with
# that reference across all of the study's outcomes, and repeat the draw 32
# times. Reproducing their scheme answers the reasonable question of whether
# the result depends on using the full set.
#
# The Article's SI describes the draw as also selecting one dependent
# variable per study, but that would yield 235 contrasts (305 conditions
# minus 70 studies), not the 469 the Article reports. Keeping all outcomes
# and drawing only the reference condition yields exactly 469 on every
# draw, matching the Article's number, so that is the scheme reproduced
# here. The stopifnot() below guards this identity.

set.seed(22)
## file-order factor levels: split() otherwise orders groups by the locale's
## collation, and the draw loop consumes the RNG stream in that order, so the
## same seed would give different draws on different machines.
idx_by_study <- split(seq_len(nrow(wide)),
                      factor(wide$study, levels = unique(wide$study)))

one_draw <- function() {
  keep <- unlist(lapply(idx_by_study, function(ii) {
    cds <- unique(c(wide$reference_condition[ii], wide$condition.name[ii]))
    ref <- cds[sample.int(length(cds), 1)]           # one reference condition
    ii[wide$reference_condition[ii] == ref | wide$condition.name[ii] == ref]
  }), use.names = FALSE)
...
stopifnot(all(draws32[, "n"] == 469L), all(draws500[, "n"] == 469L))
